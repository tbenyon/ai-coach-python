...
def add_nutrition_log(client_id, calories, protein_percent, carbs_percent, fats_percent):
    """
    Save a new nutrition entry to the database.

    Args:
        client_id: The ID of the client logging their food
        calories: Total calories consumed
        protein_percent: Percentage of calories from protein (0-100)
        carbs_percent: Percentage of calories from carbohydrates (0-100)
        fats_percent: Percentage of calories from fats (0-100)

    Returns:
        True if the nutrition log was saved successfully
    """
    ...
    # Insert the new nutrition log into the database
    cursor.execute('''
        INSERT INTO nutrition_log (client_id, date_time, calories, protein_percent, carbs_percent, fats_percent)
        VALUES (?, ?, ?, ?, ?, ?)
    ''', (client_id, current_time, calories, protein_percent, carbs_percent, fats_percent))
